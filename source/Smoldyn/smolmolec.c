...
void molsetdifc(simptr sim,int ident,enum MolecState ms,double difc) {
	...
	if(ms!=MSall) mshi=(mslo=ms)+1;
	else {mslo=0;mshi=MSMAX;}

	for(i=ilo;i<ihi;i++)
		for(ms=mslo;ms<mshi;ms++)
...
void molsetexist(simptr sim,int ident,enum MolecState ms,int exist) {
	if(ms==MSnone) return;
		...
		for(ms=0;ms<MSMAX;ms++) sim->mols->exist[ident][ms]=exist;
		return; }
...
//@Christine: changed to account for surface specific difc, put surfaceID=-1 for nosurf,
double MolCalcDifcSum(simptr sim,int i1,enum MolecState ms1,int i2,enum MolecState ms2, int surfaceID1, int surfaceID2 ) {
	double sum;
...
molssptr molssalloc(int maxspecies) {
	...
	for(i=0;i<maxspecies;i++) {
		...
		for(ms=0;ms<MSMAX;ms++) mols->difc[i][ms]=0; }
	...
	for(i=0;i<maxspecies;i++) {
		...
		for(ms=0;ms<MSMAX;ms++) mols->difstep[i][ms]=0; }
	...
	for(i=0;i<maxspecies;i++) {
		...
		for(ms=0;ms<MSMAX;ms++) mols->difm[i][ms]=NULL; }
	...
	for(i=0;i<maxspecies;i++) {
		...
		for(ms=0;ms<MSMAX;ms++) mols->drift[i][ms]=NULL; }
	...
	for(i=0;i<maxspecies;i++) {
		...
		for(ms=0;ms<MSMAX;ms++) mols->display[i][ms]=3; }
	...
	for(i=0;i<maxspecies;i++) {
		...
		for(ms=0;ms<MSMAX;ms++) mols->color[i][ms]=NULL;
	...
	for(i=0;i<maxspecies;i++) {
		...
		for(ms=0;ms<MSMAX;ms++) mols->exist[i][ms]=0; }
	...
	for(i=0;i<maxspecies;i++) {
		...
		for(ms=0;ms<MSMAX;ms++) mols->listlookup[i][ms]=-1; }
...
void molssfree(molssptr mols) {
	...
	if(mols->color) {
		for(i=0;i<maxspecies;i++)
			if(mols->color[i]) {
				for(ms=0;ms<MSMAX;ms++) free(mols->color[i][ms]);
				free(mols->color[i]); }
		free(mols->color); }
	...
	if(mols->drift) {
		for(i=0;i<maxspecies;i++)
			if(mols->drift[i]) {
				for(ms=0;ms<MSMAX;ms++) free(mols->drift[i][ms]);
				free(mols->drift[i]); }
		free(mols->drift); }
	...
	if(mols->difm) {
		for(i=0;i<maxspecies;i++)
			if(mols->difm[i]) {
				for(ms=0;ms<MSMAX;ms++) free(mols->difm[i][ms]);
				free(mols->difm[i]); }
		free(mols->difm); }
...
void writemols(simptr sim,FILE *fptr) {
	...
	for(i=1;i<mols->nspecies;i++) {
		val0=mols->difc[i][0];
		for(ms=1;ms<MSMAX && mols->difc[i][ms]==val0;ms++);
		...
		for(ms=1;ms<MSMAX && mols->listlookup[i][ms]==ll;ms++);
		...
		for(ms=1;ms<MSMAX && mols->display[i][ms]==val0;ms++);
		...
		for(ms=1;ms<MSMAX && mols->color[i][ms][0]==val0 && mols->color[i][ms][1]==val1 && mols->color[i][ms][2]==val2;ms++);
...
int checkmolparams(simptr sim,int *warnptr) {
	...
	for(i=1;i<nspecies;i++) {													// check for unused molecules
		sum=0;
		for(ms=0;ms<MSMAX;ms++) sum+=mols->exist[i][ms];
		if(!sum) {
...
/*
  Method to check, if a surface bound molecule diffused onto another surface with a different diffusion coefficient, if so
  an difstep offset is calculated (pos or neg) and returned, Christine
*/
/*double adjust_difstep(double* pos, molssptr mols, moleculeptr mptr, int dim, int mol_ndx, enum MolecState ms){
  
  panelptr pnl=mptr->pnl;
  double offset=0;
  
  if(ptinpanel(pos,pnl, dim)) return 0; //nothing to do, molecule still on same panel
  else { 
    int np,found;
    double dist, currdist;
    double newpoint[DIMMAX];
    double cur_difstep=0;
    double new_difstep=0;
    panelptr newpnl=mptr->pnl;
    
    dist=currdist=closestpanelpt(mptr->pnl->neigh[0],dim, pos, newpoint); 
    
    for (np=1; np < mptr->pnl->nneigh; np++) {  
      dist=closestpanelpt(mptr->pnl->neigh[np],dim, pos, newpoint); 
      if(dist<currdist) { 
      	newpnl=mptr->pnl->neigh[np];
        found=1; 
	currdist=dist; }
    }	
    
    cur_difstep=new_difstep=mols->difstep[mol_ndx][ms];  
    
    if (mptr->pnl->srf->sdifc[mol_ndx][ms]) cur_difstep=mptr->pnl->srf->sdifstep[mol_ndx][ms];
    if (newpnl->srf->sdifc[mol_ndx][ms]) {
      new_difstep=newpnl->srf->sdifstep[mol_ndx][ms];
    }
      
    if (found)
	//adjust difstep, only necessary if the new panel is part of another surface and the surface has its own diffusion coefficient for that molecule species
	offset=new_difstep-cur_difstep;
  }
