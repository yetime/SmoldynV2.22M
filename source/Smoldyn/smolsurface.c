...
that�s a reference and is not owned by the panel).  This is called by
...
int emittersalloc(surfaceptr srf,enum PanelFace face,int oldmaxspecies,int maxspecies) {
	...
	for(ps=0;ps<PSMAX;ps++)						// panel structure data
...
surfaceptr surfacealloc(surfaceptr srf,int oldmaxspecies,int maxspecies,int dim) {
	...
	if(!srf) {
		...
		if(!srf) return NULL;
		freesrf=1;
		srf->surface_number=-1;
		srf->sname=NULL;
		srf->srfss=NULL;
		srf->action=NULL;
		srf->actdetails=NULL;
		srf->sdifc=NULL; //Christine
		srf->sdifstep=NULL; //Christine
		...
		for(ps=0;ps<PSMAX;ps++) srf->pname[ps]=NULL;
...
void surfacefree(surfaceptr srf,int maxspecies) {
	...
	for(face=PFfront;face<=PFback;face++) {
...
surfacessptr surfacessalloc(surfacessptr srfss,int maxsurface,int maxspecies,int dim) {
	...
	if(maxsurface>srfss->maxsrf) {			// allocate any new surface names and surfaces
		...
		for(s=0;s<srfss->nsrf;s++) newnames[s]=srfss->snames[s];
		for(;s<maxsurface;s++)
...
void surfaceoutput(simptr sim) {
	...
	printf(" Surfaces allocated: %i, surfaces defined: %i, sim-nrfs %i \n\n",srfss->maxsrf,srfss->nsrf, sim->nrfs);
	for(s=0;s<srfss->nsrf;s++) {
		srf=srfss->srflist[s];
		printf(" Surface: %s\n",srfss->snames[s]);
				...
				for(ms=0;ms<MSMAX;ms++)
					for(face=0;face<3;face++)
						if(srf->action[i][ms][face]==SAmult) {
							actdetails=srf->actdetails[i][ms][face];
							if(!actdetails) {
								printf(" SMOLDYN BUG: action is SAmult but action details not allocated");error++;}
							else {
								for(ms2=0;ms2<MSMAX1;ms2++) {
									prob=actdetails->srfprob[ms2];
									if(prob<0 || prob>1) {
										error++;
										printf(" SMOLDYN BUG: surface interaction probability is <0 or >1\n"); }
									else {
...
surfaceptr surfaddsurface(simptr sim,char *surfname) {
	...
	if(s<0) {
		...
		srf=srfss->srflist[s];
		srf->surface_number=s;//Chr: added surfaces number (position in surface array)
		sim->nrfs=s+1; //Christine: Keep surface number up to date
...
int surfsetaction(surfaceptr srf,int ident,enum MolecState ms,enum PanelFace face,enum SrfAction act) {
	...
	else if(ms==MSall) {mslo=0;mshi=MSMAX-1;}
	else mslo=mshi=ms;
			...
			if(srf->nemitter[face])
				for(i=1;i<nspecies;i++)
					if(srf->nemitter[face][i]) {
						...
						//Christine: no need for use of sdifc here, surface emission probably only to solution...
						for(ps=0;ps<PSMAX;ps++)
							for(p=0;p<srf->npanel[ps];p++) {
								pnl=srf->panels[ps][p];
...
double srfcalcrate(simptr sim,surfaceptr srf,int i,enum MolecState ms1,enum PanelFace face,enum MolecState ms2) {
	...
	if(ms1==MSsoln) {															// find rates
		...
		if(face==PFnone) {
			...
			else {																			// surface to surface (flip) ->Chr: needs to account for sdifc
				if(ms1==ms2) rate=-3;											// same state
				if(srf->sdifc[i][ms1]) difc=srf->sdifc[i][ms1]; //Chr diffusion defined by state1 ms1
			...
			if(srf->sdifc[i][ms1]) difc=srf->sdifc[i][ms1]; //chr
			if((face==PFfront && ms2==MSsoln) || (face==PFback && ms2==MSbsoln))
...
surfaceptr surfreadstring(simptr sim,surfaceptr srf,char *word,char *line2,char *erstr) {
	...
	if(!strcmp(word,"name")) {								// name
		...
		ps=p=0;
		while(ps<PSMAX && (p=stringfind(srf->pname[ps],srf->npanel[ps],nm))==-1) ps++;
		...
		for(i1=0;i1<maxpnllist && (line2=strnword(line2,2));i1++) {
			...
			if(strchr(nm,':')) {
				...
				*(chptr-1)='\0';
				...
				srf2=srf; }
			ps=p=0;
			while(ps<PSMAX && (p=stringfind(srf2->pname[ps],srf2->npanel[ps],chptr))==-1) ps++;
        ...
        //Christine: To read different difc for molecules on a certain surface
        else if(!strcmp(word,"sdifc")){ 
		CHECKS(srf, "need to enter surface name before sdifc");
		CHECKS(sim->mols,"need to enter molecules before surface sdifc");
 		itct=sscanf(line2,"%s %s %i", nm, nm1, &i1);
		CHECKS(itct==3, "format for surface specific sdifc: species state difc"); 
		i=stringfind(sim->mols->spname,sim->mols->nspecies,nm);
		CHECKS(i!=-1,"in sdifc, molecule name not recognized");	
		
		/*Should check that a global difc is set as default value! If there is no global difc
		molecules are assumed to not diffuse at all*/

		if(!strcmp(nm1,"all")) {
			int j=0;
			for(j=0;j<MSMAX1;j++) {
				srf->sdifc[i][j]=i1; 
				srf->sdifstep[i][j]=sqrt(2.0*srf->sdifc[i][j]*sim->dt); }}
		else  {
			ms1=molstring2ms(nm1);
			CHECKS(ms1<=MSbsoln,"in sdifc, state1 is not recognized or not permitted");
			srf->sdifc[i][ms1]=i1;
			srf->sdifstep[i][ms1]=sqrt(2.0*srf->sdifc[i][ms1]*sim->dt); }
	}
...
int surfupdateparams(simptr sim) {
	...
	if(srfss->neighdist<0) {												// set neighdist
		difstepmax=0;
		for(i=0;i<nspecies;i++)
			for(ms1=MSfront;ms1<MSMAX;ms1++)
				if(difstep[i][ms1]>difstepmax) difstepmax=difstep[i][ms1];
				
				//Christine, accounting for the surface specific difc
		for(i=0;i<nspecies;i++){
			for(ms1=MSfront;ms1<MSMAX;ms1++){
				int j=0;
				for(j=0;j<srfss->nsrf;j++){ 
					if(srfss->srflist[j] && srfss->srflist[j]->sdifstep[i][ms1]) {
						if(srfss->srflist[j]->sdifstep[i][ms1]>difstepmax) 	difstepmax=srfss->srflist[j]->sdifstep[i][ms1]; }}}}	
		...
		for(i=1;i<nspecies;i++)
			for(ms1=0;ms1<MSMAX;ms1++)
				for(face=0;face<3;face++)
					if(srf->action[i][ms1][face]==SAmult && srf->actdetails[i][ms1][face]) {
						actdetails=srf->actdetails[i][ms1][face];
						for(ms2=0;ms2<MSMAX1;ms2++)							// record actual probability of each event
			...
			for(ms1=0;ms1<MSMAX;ms1++)
				for(face=0;face<3;face++) {
					if(srf->action[i][ms1][face]==SAmult && srf->actdetails[i][ms1][face]) {
						actdetails=srf->actdetails[i][ms1][face];
						sum=0;
						for(ms2=0;ms2<MSMAX1;ms2++)							// find ms1->ms1 probability
						...
						for(ms2=0;ms2<MSMAX1;ms2++) {						// record reverse probabilities
			...
			for(i=1;i<sim->mols->nspecies;i++)					// set srfmollist flags
				for(ms=0;ms<MSMAX;ms++) {
					...
					if(sim->mols->difc[i][ms]>0)
						srfss->srfmollist[ll]|=SMLdiffuse;
					if(rxnisprod(sim,i,ms,1))
						srfss->srfmollist[ll]|=SMLreact;
					if(ms!=MSsoln)
						srfss->srfmollist[ll]|=SMLsrfbound; }}
...
int ptinpanel(double *pt,panelptr pnl,int dim) {
	...
	//Christine: doesn't fix the cause though
	//if(!pnl) return 0;
	surfaceptr srf;
	double **point,*front;
	double len2,dot;
	...
	front = pnl->front;
		...
		if (pnl->ps == PSrect) { // rectangle
			if (dim == 1)
				...
				d = (int) front[2];
				intsct = ((point[0][d] <= pt[d] && pt[d] <= point[1][d])
						|| (point[1][d] <= pt[d] && pt[d] <= point[0][d]));
			}
			else {
				d = (int) front[2];
				intsct = ((point[0][d] <= pt[d] && pt[d] <= point[1][d])
				...
				if (d == (int) front[1])
					d = (d + 1) % 3;
				intsct = intsct && ((point[1][d] <= pt[d] && pt[d]
						<= point[2][d]) || (point[2][d] <= pt[d] && pt[d]
						<= point[1][d]));
			}
		}
		else if (pnl->ps == PStri) { // triangle
			if (dim == 1)
				...
				intsct = Geo_PtInTriangle(point[0], point[1], point[2], front,
						pt);
		}
		else if (pnl->ps == PSsph || pnl->ps == PShemi) { // sphere, hemisphere
			if (pnl->ps == PSsph)
			...
			len2 = 0;
			for (d = 0; d < dim; d++)
				len2 += (pt[d] - point[0][d]) * (pt[d] - point[0][d]);
			if (len2 <= point[1][0] * point[1][0])
				intsct = 1;
			else {
				dot = 0;
				for (d = 0; d < dim; d++)
					dot += (pt[d] - point[0][d]) * front[d];
				len2 -= dot * dot;
...
enum SrfAction surfaction(surfaceptr srf,enum PanelFace face,int ident,enum MolecState ms,int *i2ptr,enum MolecState *ms2ptr) {
	...
	if(act==SAmult) {
		...
		r=randCOD();
		ms2=MSnone;
		for(ms3=0;ms3<MSMAX1 && ms2==MSnone;ms3++)
...
int rxnXsurface(simptr sim,moleculeptr mptr1,moleculeptr mptr2) {
	...
	//Chr: Different difcs
	if(mptr1->pnl) dc1=mptr1->pnl->srf->sdifc[i1][ms1];
	if(mptr2->pnl) dc2=mptr1->pnl->srf->sdifc[i2][ms2];
...
int dosurfinteract(simptr sim,moleculeptr mptr,int ll,int m,panelptr pnl,enum PanelFace face,double *crsspt) {
	int done,dim,d,i,i2,isneigh,p;
	enum PanelFace newface;
	enum MolecState ms,ms2;
	enum SrfAction act;
	double x,norm[DIMMAX],epsilon,neighdist;
	...
	if(act==SAno);																// no action
		...
		if(ms!=MSsoln) movemol2closepanel(sim,mptr,dim,epsilon,neighdist);
